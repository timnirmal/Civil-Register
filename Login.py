...
class main:
    ...
    def login(self):
        ...
        find_user = ('SELECT * FROM user WHERE name = ? and password = ?')
        c.execute(find_user, [(self.name.get()), (self.password.get())])
        result = c.fetchall()
        if result:
            self.logf.pack_forget()
            self.head['text'] = self.name.get() + '\n Loged In'
            self.head['pady'] = 150
        else:
    ...
    def new_user(self):
        ...
        find_user = ('SELECT name FROM user WHERE name = ?')
        c.execute(find_user, [(self.n_name.get())])
        if c.fetchall():
        ...
        c.execute(insert, [(self.n_name.get()), (self.n_password.get())])
        db.commit()
    ...
    def widgets(self):
        ...
        Entry(self.logf, textvariable=self.name, bd=5, font=('', 15)).grid(row=0, column=1)
